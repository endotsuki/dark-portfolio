        ...
        {data.map((item, index) => (
          <div key={index} className='flex justify-start pt-10 md:gap-10 md:pt-40'>
            <div className='sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm'>
              <div className='absolute left-3 flex h-10 w-10 items-center justify-center rounded-full bg-slate-950 md:left-3'>
                <div className='h-4 w-4 rounded-full border border-neutral-700 bg-neutral-800 p-2' />
              </div>
              <h3 className='hidden text-xl font-medium text-neutral-500 md:block md:pl-20 md:text-4xl'>{item.title}</h3>
            </div>

            <div className='relative w-full pl-20 pr-4 md:pl-4'>
              <h3 className='mb-4 block text-left text-2xl font-medium text-neutral-500 md:hidden'>{item.title}</h3>
              {item.content}{' '}
            </div>
          </div>
        ))}
